...
print("Reloading model with new dataset...")
routes.init_thermal_cvd_model()
opt = routes.optimizer_instance

print("Running BO loop for 5 iterations...")
for i in range(5):
    res = opt.simulate_experiment()
...
ei_vals = opt.bo_engine.expected_improvement(
...
# Plot EI
plt.subplot(2, 1, 2)
plt.plot(gte_range, ei_vals, 'g-', label='Expected Improvement')
plt.title('Acquisition Function')
plt.xlabel('Growth Temp (GTE) °C')
plt.ylabel('EI')
plt.legend()

plt.tight_layout()
artifact_dir = Path("C:/Users/Khushboo/.gemini/antigravity/brain/47ac04c3-e644-433a-886e-f4f6f1878339")
plt.savefig(artifact_dir / "bo_plot.png")
