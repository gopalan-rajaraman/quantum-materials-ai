...
import { 
  ...
  User, ChevronDown, ArrowRight, ChevronLeft, ChevronRight, Share2, Target, AlertCircle, Loader2, ExternalLink
} from 'lucide-react';
import { 
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#4C3BDE] hover:bg-[#3b2eb5] text-white font-semibold rounded-lg text-sm shadow-md shadow-indigo-100 transition-colors"
          >
            <ExternalLink className="w-4 h-4" /> Generate Full Report
          </button>
        </div>
      </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-100">
                      ...
                      <th className="text-left py-3 px-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredReports.map((report, idx) => (
                      <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-4 px-2">
                          <button
                            onClick={() => navigate('/reports/full')}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#F4F0FF] hover:bg-[#EDE8FF] text-[#4C3BDE] text-[11px] font-bold rounded-lg transition-colors"
                          >
